...
exports.generateComponentTreeDiagram = generateComponentTreeDiagram;
/**
...
function generateMermaidDiagram(analysis) {
    ...
    const usedComponents = [
        ...new Set([...jsx, ...components.map((c) => c.name)]),
    ];
    // 컴포넌트 유형에 따른 색상 정의
    const styles = {
    ...
    styleDefinitions.push(`    style T fill:${styles.template.fill},stroke:${styles.template.stroke}`);
    // 각 컴포넌트 타입별로 그룹화
    ...
    componentsByType.organism.forEach((comp, i) => {
        ...
        styleDefinitions.push(`    style ${nodeId} fill:${styles.organism.fill},stroke:${styles.organism.stroke}`);
    });
    // Molecule 컴포넌트 노드 추가
    componentsByType.molecule.forEach((comp, i) => {
        ...
        styleDefinitions.push(`    style ${nodeId} fill:${styles.molecule.fill},stroke:${styles.molecule.stroke}`);
    });
    // Atom 컴포넌트 노드 추가
    componentsByType.atom.forEach((comp, i) => {
        ...
        styleDefinitions.push(`    style ${nodeId} fill:${styles.atom.fill},stroke:${styles.atom.stroke}`);
    });
    // 알 수 없는 컴포넌트 노드 추가
    componentsByType.unknown.forEach((comp, i) => {
        ...
        connections.push(`    T --> ${nodeId}`);
        styleDefinitions.push(`    style ${nodeId} fill:${styles.unknown.fill},stroke:${styles.unknown.stroke}`);
    });
    // 다이어그램 생성
    const diagram = [
        "graph TD",
 ...
 */
function generateComponentTreeDiagram(rootComponent) {
    if (!rootComponent) {
        return 'graph TD\n    Root["컴포넌트 트리 없음"]';
    }
    // 스타일 정의
    ...
    function traverseComponent(component, parentId) {
        ...
        styleDefinitions.push(`    style ${id} fill:${style.fill},stroke:${style.stroke}`);
        // 하위 컴포넌트 순회
