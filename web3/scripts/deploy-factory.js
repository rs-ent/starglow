...
function getGasSettingsFromArgs() {
    ...
    // 명령줄 인자에서 가스 설정 읽기
    if (args["max-fee"]) {
        settings.maxFeePerGas = (0, viem_1.parseGwei)(args["max-fee"]);
    }
    if (args["max-priority-fee"]) {
        settings.maxPriorityFeePerGas = (0, viem_1.parseGwei)(args["max-priority-fee"]);
    }
    if (args["gas-limit"]) {
    ...
    try {
        ...
        if (forceDefaultGas) {
            ...
            if (Object.keys(gasSettings).length === 0 &&
                ...
                console.error("\nERROR: Gas settings are required for deployment to network:", networkName);
                console.error("Please provide the following command line arguments:");
                console.error("  --max-fee <gwei>         - Maximum fee per gas in gwei");
                console.error("  --max-priority-fee <gwei> - Maximum priority fee per gas in gwei");
                console.error("  --gas-limit <limit>      - Gas limit for deployment");
                console.error("\nExample:");
                console.error(`  npx hardhat run scripts/deploy-factory.ts --network ${networkName} --max-fee 3 --max-priority-fee 2 --gas-limit 4000000`);
                console.error("\nOr use --use-default-gas to use default gas settings");
                if (defaultGasSettings[networkName]) {
                    console.error("\nDefault gas settings for", networkName, ":");
                    console.error("  --max-fee", Number(defaultGasSettings[networkName].gasSettings
                        .maxFeePerGas || 0n) / 1e9, "gwei");
                    console.error("  --max-priority-fee", Number(defaultGasSettings[networkName].gasSettings
                        .maxPriorityFeePerGas || 0n) / 1e9, "gwei");
                    console.error("  --gas-limit", defaultGasSettings[networkName].gasSettings.gasLimit?.toString());
                }
                process.exit(1);
            }
        }
        ...
        if (Object.keys(gasSettings).length > 0) {
            ...
            if (gasSettings.maxFeePerGas) {
                console.log(`- Max Fee: ${(0, viem_1.formatEther)(gasSettings.maxFeePerGas)} ETH (${gasSettings.maxFeePerGas} wei)`);
            }
            if (gasSettings.maxPriorityFeePerGas) {
                console.log(`- Max Priority Fee: ${(0, viem_1.formatEther)(gasSettings.maxPriorityFeePerGas)} ETH (${gasSettings.maxPriorityFeePerGas} wei)`);
            }
        ...
        // 검증 설정
        const verification = defaultGasSettings[networkName]?.verification !== false &&
            !skipVerification;
        // 최종 설정 적용
        const currentConfig = {
            gasSettings,
        ...
        const Factory = await hardhat_1.default.ethers.getContractFactory("CollectionFactory");
        const deployTx = await Factory.deploy(deployer.account.address, currentConfig.gasSettings);
        ...
        const deploymentInfo = {
            ...
            gasSettings: {
                maxFeePerGas: currentConfig.gasSettings.maxFeePerGas?.toString(),
                maxPriorityFeePerGas: currentConfig.gasSettings.maxPriorityFeePerGas?.toString(),
        ...
        if (!fs_1.default.existsSync(deploymentsDir)) {
            fs_1.default.mkdirSync(deploymentsDir);
        }
        const networkDir = path_1.default.join(deploymentsDir, networkName);
        if (!fs_1.default.existsSync(networkDir)) {
            fs_1.default.mkdirSync(networkDir);
        }
        fs_1.default.writeFileSync(path_1.default.join(networkDir, "Factory.json"), JSON.stringify(deploymentInfo, null, 2));
        // 컨트랙트 검증
        if (currentConfig.verification &&
            networkName !== "hardhat" &&
            networkName !== "localhost") {
            ...
            try {
                ...
                console.log(`Contract address: ${factoryAddress}`);
                console.log(`Constructor arguments: [${deployer.account.address}]`);
            }
        }
        console.log(`\nDeployment completed! 🚀`, `\n- Network: ${networkName}`, `\n- Chain ID: ${chainId}`, `\n- Factory address: ${factoryAddress}`, `\n- Transaction hash: ${deployTx.hash}`, `\n- Deployment info saved to: ${path_1.default.join(networkDir, "Factory.json")}`);
